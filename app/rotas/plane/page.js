...
const RotasAviaoPage = () => {
  ...
  const [newRoute, setNewRoute] = useState({
    ...
    base_price: '',
    total_seats: '',
    flight_number: '', // New field for plane routes
    airline_code: '' // New field for plane routes
  });
...
const fetchCompanies = async () => {
  ...
  const fetchRoutes = async () => {
    const { data, error } = await supabase
      .from('plane_routes') // Changed from bus_routes
      .select('*, plane_companies(name)'); // Changed from bus_companies
  ...
  const handleAddRoute = async () => {
    const { data, error } = await supabase
      ...
      .insert({
        ...
        type: 'plane' // Set the type explicitly for plane routes
      });
  ...
  <div className="flex justify-between mb-4">
    <h1 className="text-2xl font-bold">Rotas de Avião</h1> {/* Changed title */}
    <Dialog open={isAddRouteOpen} onOpenChange={setIsAddRouteOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Adicionar Nova Rota de Avião</DialogTitle> {/* Changed title */}
        </DialogHeader>
  ...
  <div className="bg-white rounded-lg shadow-md">
      ...
      <table className="w-full">
        <thead className="bg-gray-50">
          <tr>
            ...
            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Voo</th> {/* Changed header */}
        ...
        <tbody className="divide-y divide-gray-200">
          {routes.map((route) => (
            ...
            >
              ...
              <td className="px-6 py-4 text-sm">{route.flight_number} ({route.airline_code})</td> {/* Display flight info */}
    ...
    {selectedRoute && (
      <div className="bg-white p-4 border-t">
        ...
        <div className="flex items-center space-x-4 mt-2">
          <Plane size={24} className="text-gray-500" /> {/* Changed icon */}
        ...
        <div className="flex items-center space-x-4 mt-2">
          <span className="text-lg font-medium">Voo: {selectedRoute.flight_number} ({selectedRoute.airline_code})</span> {/* Display flight info */}
        </div>
      </div>
    )}
  </div>
